...
// ─── Contact Form (unchanged from BookingCTA) ─────────────────────────────────
...
function InstantQuoteForm() {
  ...
  // Clamp step index if active steps shrink (e.g. switching from Gold to Basic)
  useEffect(() => {
    if (step >= activeSteps.length) setStep(activeSteps.length - 1);
  }, [activeSteps.length, step]);

  // Scroll to top of card on mobile when step advances
  useEffect(() => {
    if (!hasMounted.current) { hasMounted.current = true; return; }
    if (!formCardRef.current || window.innerWidth >= 768) return;
  ...
  return (
    ...
    >
      {/* Tab toggle */}
      <TabToggle activeTab="quote" onSwitch={() => {}} />

      {submitted && currentStepId === "contact" ? (
        <StepContact form={form} setForm={setForm} onSubmit={() => setSubmitted(true)} submitted={submitted} />
      ) : (
        <>
          ...
          {/* Navigation — hidden on contact step (submit button is inside) */}
...
// ─── TabToggle (shared between InstantQuote and Contact panels) ───────────────

function TabToggle({ activeTab, onSwitch }: { activeTab: "quote" | "contact"; onSwitch: (t: "quote" | "contact") => void }) {
  return (
...
export default function HomepageBookingCTA({
  ...
  const handleTabSwitch = (t: "quote" | "contact") => setTab(t);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr]" style={{ gap: "clamp(40px, 5vw, 80px)" }}>
          ...
          <RevealOnScroll>
            <div>
              ...
              <div className="flex flex-col" style={{ gap: 32 }}>
                {[
                  { icon: "M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z", label: "Phone", value: "(403) 448-4048" },
          ...
          <RevealOnScroll delay={200}>
            <div style={{
              ...
              <div style={{ display: "flex", background: "#f3f4f6", borderRadius: 10, padding: 4, marginBottom: 28 }}>
                {(["quote", "contact"] as const).map(t => (
                  <button key={t} type="button" onClick={() => handleTabSwitch(t)}
                    style={{
                      flex: 1, padding: "9px 0", borderRadius: 7, border: "none",
...
function QuotePanel() {
  ...
  // Clamp if form changes shrink the step list
  useEffect(() => {
    if (step >= activeSteps.length) setStep(Math.max(0, activeSteps.length - 1));
  }, [activeSteps.length, step]);

  // Mobile scroll on advance
  useEffect(() => {
    if (!hasMounted.current) { hasMounted.current = true; return; }
